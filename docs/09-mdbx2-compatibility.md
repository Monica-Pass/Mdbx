...
The repository freezes `crates/mdbx-storage/test-data/mdbx1-release-1.0.mdbx`, generated by the historical `MDBX1.0` tag at commit `1a43fa9e8e87eebf6d0e1b84543c3291d0b25142`. Its manifest records the immutable SHA-256, test-only unlock credential, and stable project, entry, attachment, and snapshot IDs.

The migration regression copies those exact bytes, verifies that inspection is read-only, upgrades schema 1 to the current schema, unlocks with the original MDBX1 credential, and compares the legacy commit and object-version identities before and after. It also verifies project metadata, entry payload, project tags, inline attachment bytes, snapshot identity, and repeated-upgrade idempotence.
