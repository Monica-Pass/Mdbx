...
- `mdbx-ffi` 已进入 workspace，作为非 Rust 客户端的通用 UniFFI 边界；当前覆盖 vault/project/generic entry、完整 Tiga2 策略读取与逐操作授权、真实设备能力、迁移检查/显式升级、显式 Tiga 创建、安全密钥材料解锁、组合因素整改、已解锁状态下重设主密码，以及 Collection/Object/Attachment/Conflict 的有界导航。后续 tag、sync、snapshot、diagnostics 等跨语言能力应继续扩展 facade，而不是让客户端直接写表。
- UniFFI 读取已区分 metadata-only selection 与 Tiga disclosure：`get_object_summary`/`list_object_summaries`、`list_deleted_object_summaries`、`list_all_deleted_object_summaries`、relation/label/assignment summary 页面和 `list_unresolved_conflict_summaries` 不读取业务 payload；冲突摘要还限制字段 JSON、路径数和游标，并绑定对象类型过滤。`reveal_object*` 仅在 Entry 决定允许后返回 `ObjectRecord`；`reveal_object_relation*` 同时保留 source/target Entry 决定；`reveal_object_label*` 继承 collection Project 决定。非允许结果保留类型化原因与约束且不含 payload；旧完整读取继续作为兼容接口。
