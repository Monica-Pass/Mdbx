...
- 已删除对象不得披露 payload。
- 允许结果必须连同 `AuthorizationDecision` 返回，客户端必须执行其中的约束；拒绝结果必须以授权错误返回，不能因为 payload 已损坏而先暴露密码学错误。
...
策略求值、允许后的对象读取和成功审计应位于同一个存储事务中，避免策略与对象在判定和解密之间发生切换。拒绝决定必须在不读取对象密文字段的情况下完成并保留审计事件。
