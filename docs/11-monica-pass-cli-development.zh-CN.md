...
- PIN / 密码 / security_key / password_security_key storage core 支持。
- 中文密码 NFC 规范化测试。
...
参考 `mdbx-cli` 已把默认详情与秘密披露拆开：

```text
mdbx entry get <entry-id>
mdbx entry get <entry-id> --reveal
```
...
- 兼容层仍保留可返回完整 payload 的低层读取 API；新客户端若绕过 `ObjectDisclosureService`，仍可能重新引入授权前明文风险。
...
- 参考 `mdbx-cli entry get` 默认只读对象摘要并明确显示 payload 已脱敏；显式 `--reveal` 通过 Tiga `RevealSecret` 披露服务。
- Tiga project/entry 策略解析使用严格的 metadata-only 上下文；无效覆盖失败关闭，拒绝决定先于 payload 解密。
